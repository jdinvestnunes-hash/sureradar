...
async def lifespan(app):
    ...
    try:
        promo.iniciar()               # fluxo de marketing no grupo (self-gate: PROMO_ATIVO)
    except Exception as e:
...
def admin_recuperar_pendentes(request: Request):
    """Força a RÉGUA AUTOMÁTICA de recuperação rodar agora (sem esperar o tick de 1h).
    A recuperação já é automática: quem gera checkout (Pix/cartão) e não paga entra na
    régua do recuperacao.py — 1º e-mail ~1h após gerar, puxando o e-mail do NOSSO banco.
    Este botão só antecipa essa rodada; reusa o mesmo dedup/orçamento, então nunca duplica."""
    import threading
    import recuperacao
    user = _usuario(request)
    erro = _guard_admin(request, user)
    ...
    if not config.RESEND_API_KEY:
        return JSONResponse({"erro": "RESEND_API_KEY não configurada"}, status_code=503)
    # público atual (só pra devolver quantos estão elegíveis agora — a régua decide o resto)
    excluir = _emails_excluidos(user)
    elegiveis = {(u.get("email") or "").strip().lower()
                 for u in auth.usuarios_para_recuperacao()} - excluir

    threading.Thread(target=recuperacao._rodar_uma_vez, name="recup-forcar",
                     daemon=True).start()
    return {"ok": True, "audiencia": len(elegiveis)}
